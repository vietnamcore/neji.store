...
-- =========================================================
-- MIGRATION 7 — Đồng bộ toàn hệ thống (Thuê / Đặt lịch / Dashboard /
-- Tra cứu / Lịch sử / Thông báo / Doanh thu / Trạng thái Account dùng
-- chung 1 nguồn dữ liệu) + bảng giảm giá cấu hình được, không ghi cứng.
-- Idempotent, additive, không mất dữ liệu, không xoá chức năng cũ.
-- =========================================================

-- 7.1 Bổ sung trạng thái 'cancelled' cho orders — giao diện đã dùng
--     trạng thái này (Hủy đơn) nhưng check constraint gốc còn thiếu.
alter table public.orders drop constraint if exists orders_status_check;
alter table public.orders add constraint orders_status_check
  check (status in ('holding','unconfirmed','confirmed','rejected','expired','cancelled'));

-- 7.2 Thông tin khách hàng cho đơn "Thuê ngay" — đồng bộ với bookings
--     để Dashboard / Tra cứu / Lịch sử hiển thị Tên khách, SĐT cho
--     TẤT CẢ đơn (không chỉ riêng Đặt lịch).
...
-- Mã chuyển khoản: với "Thuê ngay" trùng với code đơn; lưu riêng cột
-- để đồng nhất cấu trúc hiển thị với Đặt lịch (nơi 2 mã có thể khác nhau).
alter table public.orders add column if not exists transfer_code text;
create index if not exists idx_orders_phone on public.orders(phone);

-- 7.3 Mã chuyển khoản riêng cho Đặt lịch — mã này là mã khách ghi khi
--     chuyển khoản giữ chỗ, có thể phát sinh trước khi booking được
--     tạo chính thức (mã booking `code` do RPC create_booking sinh ra
--     ngay sau khi thanh toán được xác nhận là đã gửi).
alter table public.bookings add column if not exists transfer_code text;
create index if not exists idx_bookings_transfer_code on public.bookings(transfer_code);

-- 7.4 create_booking: nhận thêm mã chuyển khoản tạm (p_transfer_code)
--     để lưu lại đúng mã khách đã thấy trên Popup Thanh toán.
create or replace function public.create_booking(
  p_account_id     text,
  p_account_name   text,
  p_customer_name  text,
...
begin
  ...
  begin
    insert into public.bookings (
      code, account_id, account_name, customer_name, phone, telegram, zalo,
      start_time, end_time, duration_hours, amount, notes, status, transfer_code
    ) values (
      v_code, p_account_id, p_account_name, p_customer_name, p_phone, p_telegram, p_zalo,
      p_start_time, p_end_time, p_duration_hours, p_amount, p_notes, 'pending',
      coalesce(p_transfer_code, v_code)
    )
    returning * into v_booking;
  exception
    when exclusion_violation then
...
grant execute on function public.create_booking(text,text,text,text,text,text,timestamptz,timestamptz,numeric,numeric,text,text) to anon, authenticated;

-- 7.5 BẢNG GIẢM GIÁ DÙNG CHUNG — không ghi cứng % giảm trong code.
-- Admin chỉnh sửa min_hours / percent ngay trong tab "Giảm giá" của
-- Dashboard mà KHÔNG cần sửa code. index.html nạp bảng này lúc khởi
-- động và tính giá qua đúng 1 hàm getPackagePrice() dùng chung cho
-- Card / Popup Thuê / Popup Đặt lịch / Dashboard.
...
create unique index if not exists idx_discount_tiers_min_hours on public.discount_tiers(min_hours);

drop trigger if exists trg_discount_tiers_updated_at on public.discount_tiers;
create trigger trg_discount_tiers_updated_at
  before update on public.discount_tiers
  for each row execute function public.set_updated_at();

insert into public.discount_tiers (min_hours, percent, label, sort_order) values
  (6,  10, 'Từ 6 giờ',          1),
  (12, 20, 'Từ 12 giờ',         2),
  (24, 30, 'Từ 24 giờ',         3),
  (48, 35, 'Từ 48 giờ',         4),
  (72, 40, 'Từ 72 giờ trở lên', 5)
on conflict (min_hours) do nothing;
...
create policy discount_tiers_select_all on public.discount_tiers for select using (true);
drop policy if exists discount_tiers_write_all on public.discount_tiers;
create policy discount_tiers_write_all on public.discount_tiers for all using (true) with check (true);

do $$
begin
  begin
    alter publication supabase_realtime add table public.discount_tiers;
  exception when duplicate_object then null;
  end;
  begin
    alter publication supabase_realtime add table public.orders;
...
-- =========================================================
-- HẾT MIGRATION 7 — Sau khi chạy, vào Database > Replication và bật
-- Realtime cho discount_tiers nếu chưa tự bật.
-- =========================================================
